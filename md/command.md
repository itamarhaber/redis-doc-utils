...
Here are a few examples of commands that are marked with `movablekeys`:
  ...
  - [`ZUNION`](/commands/zunion) -  keys stop after `numkeys` count arguments
  - [`ZUNIONSTORE`](/commands/zunionstore) -  keys stop after `numkeys` count arguments
  - [`ZINTER`](/commands/zinter) - keys stop after `numkeys` count arguments
  - [`ZINTERSTORE`](/commands/zinterstore) -  keys stop after `numkeys` count arguments
  - [`MIGRATE`](/commands/migrate) - keys start after the [`KEYS`](/commands/keys) keyword (if the second argument is en empty string)
...
The only two commands which still require using `COMMAND GETKEYS` are [`SORT`](/commands/sort) and [`MIGRATE`](/commands/migrate) (assuming the client can parse and uses the `key-specs` section).
For more information please check the [key-specs page][tr].
[tr]: /topics/key-specs
